...
#define BUTTON_PIN 32 // Example pin, choose an available pin
 
// EEPROM addresses
#define EEPROM_SIZE 128 // Increased EEPROM size to accommodate more settings
...
#define DEFAULT_OVER_CURRENT_THRESHOLD 100.0
...
#define ORANGE    0xFD20 // Example Orange
...
void updateDisplayValues() {
    ...
    if (notificationEndTime > 0 && millis() < notificationEndTime) {
        if (strcmp(currentNotification.c_str(), lastNotificationText) != 0 ||
            millis() - lastNotificationUpdate > 500) {
...
bool newFaultState = (readings.voltage > overVoltageThreshold ||
                      readings.current > overCurrentThreshold);
...
if (server.arg("username") == admin_username &&
